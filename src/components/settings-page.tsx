...
export function SettingsPage({ profile, feedbackPage, onBack, onSaveProfile, onReset }: { profile: UserProfile; feedbackPage: string; onBack: () => void; onSaveProfile: (profile: UserProfile) => void; onReset: () => void }) {
  ...
  const updateBirthTime = (birthTime: string) => setDraft((previous) => ({ ...previous, birthTime, birthTimeUnknown: false }));
  const setBirthTimeUnknown = (birthTimeUnknown: boolean) => setDraft((previous) => ({ ...previous, birthTimeUnknown, birthTime: birthTimeUnknown ? "" : previous.birthTime }));
  const saveProfile = () => { if (!draft.nickname.trim() || !draft.birthday || !draft.location.trim() || (!draft.birthTime && !draft.birthTimeUnknown)) { setMessage("请完成必填资料，或选择不知道出生时间。"); return; } onSaveProfile({ ...draft, nickname: draft.nickname.trim(), location: draft.location.trim(), birthTime: draft.birthTimeUnknown ? "" : draft.birthTime }); setMessage("出生资料已更新。"); };
  ...
  return <section className="page">
    ...
    <div className="card"><p className="eyebrow" style={{ marginTop: 0 }}>测试档案</p><label className="form-label">昵称<input className="field" value={draft.nickname} onChange={(event) => update("nickname", event.target.value)} /></label><label className="form-label">出生日期<input className="field" type="date" max={localToday()} value={draft.birthday} onChange={(event) => update("birthday", event.target.value)} /></label><label className="form-label">出生时间<input className="field" type="time" disabled={draft.birthTimeUnknown} value={draft.birthTime} onChange={(event) => updateBirthTime(event.target.value)} /></label><label className="check-row"><input type="checkbox" checked={draft.birthTimeUnknown} onChange={(event) => setBirthTimeUnknown(event.target.checked)} /> <span>不知道出生时间</span></label><label className="form-label">出生城市<input className="field" value={draft.location} onChange={(event) => update("location", event.target.value)} /></label><button className="secondary" style={{ width: "100%", marginTop: 18 }} onClick={saveProfile}>保存出生资料</button><button className="danger-button" style={{ width: "100%", marginTop: 10 }} onClick={reset}>清除档案并重新开始</button></div>

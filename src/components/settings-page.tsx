...
export function SettingsPage({ profile, feedbackPage, onBack, onSaveProfile, onReset }: { profile: UserProfile; feedbackPage: string; onBack: () => void; onSaveProfile: (profile: UserProfile) => void; onReset: () => void }) {
  ...
  const saveProfile = () => { if (!draft.nickname.trim() || !draft.birthDate || !draft.birthCity.trim() || (!draft.birthTime && !draft.birthTimeUnknown)) { setMessage("请完成必填资料，或选择不知道出生时间。"); return; } onSaveProfile({ ...draft, nickname: draft.nickname.trim(), birthCity: draft.birthCity.trim(), birthTime: draft.birthTimeUnknown ? "" : draft.birthTime }); setMessage("出生资料已更新。"); };

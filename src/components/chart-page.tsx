...
export function ChartPage({ profile, onFeedback }: { profile: UserProfile; onFeedback: () => void }) {
  const content = useDemoContent(profile);
  const chartSections = content?.chart;
  if (!chartSections) return <section className="page" aria-busy="true" />;
